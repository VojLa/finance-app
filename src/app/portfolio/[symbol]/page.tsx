...
export default function SymbolPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`/api/portfolio/transactions?symbol=${encodeURIComponent(symbol)}`).then((r) =>
        r.json()
      ),
    ]).then(([portfolio, txs]) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <p
            className={`text-xl font-semibold font-mono ${pnlPos ? "text-green-600" : "text-red-600"}`}
          >
              ...
              <tbody className="divide-y divide-gray-50">
                {transactions.map((tx) => (
                  <tr key={tx.id} className="hover:bg-gray-50">
                    ...
                    <td className="py-2.5">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-medium ${TYPE_BADGE_COLORS[tx.type] ?? "bg-gray-50 text-gray-600"}`}
                      >
                    ...
                    <td className="py-2.5 text-right font-mono text-gray-500">
                      {tx.pricePerUnit !== null
                        ? `${fmt(tx.pricePerUnit)} ${tx.priceCurrency ?? ""}`
                        : "—"}
                    </td>
                    <td className="py-2.5 text-right font-mono font-medium">
                      {tx.totalAmount !== null
                        ? `${fmt(tx.totalAmount)} ${tx.totalCurrency ?? ""}`
                        : "—"}
                    </td>
                    ...
                    <td
                      className={`py-2.5 text-right font-mono ${tx.realizedPnl !== null && tx.realizedPnl >= 0 ? "text-green-600" : tx.realizedPnl !== null ? "text-red-600" : "text-gray-300"}`}
                    >
                      {tx.realizedPnl !== null
