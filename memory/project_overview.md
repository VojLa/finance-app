...
export async function recalculateHoldings(accountId: string) {
  ...
  for (const tx of buyTransactions) {
    ...
    if (!positions[tx.symbol])
      positions[tx.symbol] = { quantity: 0, totalCost: 0, currency: tx.priceCurrency || "EUR" }
  ...
  for (const tx of sellTransactions) {
    ...
    const avgPrice =
      positions[tx.symbol].totalCost / (positions[tx.symbol].quantity + (tx.quantity || 0))
